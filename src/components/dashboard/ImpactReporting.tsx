...
import { Heart, TrendingUp, Users, FileText, Loader2, Download, Building2 } from "lucide-react";
...
export function ImpactReporting() {
  ...
  const handleGenerateCertificate = async () => {
    ...
    try {
      const certificateId = `IMP-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 99999)).padStart(5, '0')}`;
      const impactUnits = Math.floor(getMetricValue("impact_units") * 0.4);
      const livesImpacted = Math.floor(getMetricValue("beneficiaries_served") * 0.4);
  ...
  // Dynamic impact stats - configurable per tenant
  const impactStats = [
    {
      type: "impact_units",
      fallbackType: "liters_filtered",
      icon: TrendingUp,
      label: terminology.impactUnitLabel,
      subtext: "Since inception",
      color: "from-blue-500 to-cyan-500",
      bgColor: "bg-blue-500/10",
    },
    {
      type: "beneficiaries_served",
      fallbackType: "children_served",
      icon: Users,
    ...
    {
      type: "organizations_supported",
      fallbackType: "schools_equipped",
      icon: Building2,
      label: terminology.communitiesLabel,
      subtext: "Supported",
  ...
  return (
    <FeatureGuard feature="impact" featureName="Impact Reporting">
      <motion.div
      ...
      >
        ...
        <div className="bg-card rounded-xl border p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-foreground">{terminology.impactLabel}</h2>
              <p className="text-sm text-muted-foreground">{terminology.impactDescription}</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {impactStats.map((stat, index) => {
              // Try to get primary metric, fall back to legacy metric
              const value = getMetricValue(stat.type) || getMetricValue(stat.fallbackType);
              ...
              return (
                <motion.div
                  ...
                  className={`relative rounded-xl border ${stat.bgColor} p-6 overflow-hidden`}
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-sm text-muted-foreground/70">{stat.subtext}</p>
                  </div>
                </motion.div>
              );
            })}
        ...
        <div className="bg-card rounded-xl border p-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-foreground mb-1">Impact Certificate Generator</h3>
              <p className="text-sm text-muted-foreground">
                Generate verified impact certificates for partners and donors
              </p>
            </div>
            <Button
                ...
                <div className="bg-white rounded-lg p-8 text-slate-900 border">
                  ...
                  <div className="grid grid-cols-2 gap-6 my-8">
                    <div className="text-center p-4 bg-primary/5 rounded-lg">
                      ...
                      <p className="text-sm text-slate-600">{terminology.impactUnitLabel}</p>
                    </div>
                    <div className="text-center p-4 bg-primary/5 rounded-lg">
                      <p className="text-3xl font-bold text-primary">
